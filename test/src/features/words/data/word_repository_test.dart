...
void main() {
  group('WordRepo', () {
    ...
    group('addWord', () {
      test(
        'addWord calls Dio.post and succeeds and fetch the right data',
        () async {
      ...
      test(
        'addWord calls Dio.post but failed and sends an Exception and a null data',
        () async {
          const wordToAdd = Word(
            id: '',
            trad: {'fr': 'rire', 'en': 'laugh'},
    ...
    group('delWord', () {
      test('delWord calls Dio.delete and succeeds', () async {
        ...
        verify(
          () => mockDio.delete('/voc/word/5', options: any(named: 'options')),
        ).called(1);
      });
      test('delWord calls Dio.delete but failed and does not throw', () async {
        when(
          () => mockDio.delete(any(), options: any(named: 'options')),
        ).thenAnswer(
          (_) async => Response(
    ...
    group('fetchWordsBy', () {
      ...
      test(
        'fetchWordsByIds calls Dio.get(list) but failed and returns an empty list',
        () async {
          const listIds = <String>['1', '2', '3'];
          when(
            () => mockDio.get(
    ...
    group('fetchAllWords', () {
      ...
      test(
        'fetchAllWords calls Dio.get but failed and returns an empty list',
        () async {
          when(
            () => mockDio.get(any(), options: any(named: 'options')),
          ).thenAnswer(
